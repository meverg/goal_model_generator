...
int main(int argc, char **argv)
{
    ...
    msat_term tlower;
    if (opts.local_lower) {
        tlower = msat_make_number(env, opts.local_lower);
        if (MSAT_ERROR_TERM(tlower)) {
            fprintf(stderr, "Error in converting '%s' to a msat_term number.\n", opts.local_lower);
        }
    } else {
        MSAT_MAKE_ERROR_TERM(tlower);
    }
    msat_term tupper;
    if (opts.local_upper) {
        tupper = msat_make_number(env, opts.local_upper);
        if (MSAT_ERROR_TERM(tupper)) {
            fprintf(stderr, "Error in converting '%s' to a msat_term number.\n", opts.local_upper);
        }
    } else {
        MSAT_MAKE_ERROR_TERM(tupper);
    }

    /* create objective */
    fprintf(stdout, "Minimizing `%s' ...\n", opts.objective);
    msat_objective o = msat_push_minimize(env, cost, tlower, tupper);
