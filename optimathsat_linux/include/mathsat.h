 ...
 * 
 * OptiMathSAT is copyrighted 2009-2018 by Fondazione Bruno Kessler, Trento, Italy,
...
typedef enum {
    ...
    MSAT_TAG_FP_TO_BV,      /**< FP conversion to BV */
...
typedef enum {
    MSAT_OPTIMUM = 0,
    MSAT_INITIAL_LOWER = 1,
    MSAT_INITIAL_UPPER = 2,
    MSAT_FINAL_LOWER = 3,
    MSAT_FINAL_UPPER = 4,
    MSAT_FINAL_ERROR = 5,
    MSAT_FINAL_TOLERANCE = 6,
 ...
 * \return zero on success, nonzero on error
 */
int msat_set_external_maxsat_engine(msat_env env,
                                        msat_ext_maxsat_interface *engine);

/*@}*/ /* end of external MaxSAT - Lemma Lifting group */


/**
 * \name OptiMathSAT - objectives creation
 */
/*@{*/

/**
 * \brief Push on the stack the new objective 'min(term)' with optional
 * optimization local interval [lower, upper[
 *
 * \param e The environment in which to operate.
 * \param term  The term to be minimized.
 * \param lower The initial lower bound.
 * \param upper The initial upper bound.
 * \param bvsigned True iff bitvector goal is signed (default: false)
 *
 * \return An objective, or a o s.t. ::MSAT_ERROR_OBJECTIVE(o) is true
 * in case of errors.
 */
msat_objective msat_push_minimize(msat_env e, msat_term term, msat_term lower,
                                  msat_term upper, bool bvsigned = false);

/**
 * \brief Push on the stack the new objective 'max(term)' with optional
 ...
 * in case of errors.
 */
msat_objective msat_push_maximize(msat_env e, msat_term term, msat_term lower,
                                  msat_term upper, bool bvsigned = false);

/**
 * \brief Push on the stack the new objective 'min(max(term0), ..., max(termN))'
 ...
 * in case of errors.
 */
msat_objective msat_push_minmax(msat_env e, size_t len, msat_term terms[],
                                msat_term lower, msat_term upper,
                                bool bvsigned = false);

/**
 * \brief Push on the stack the new objective 'max(min(term0), ..., min(termN))'
 * with optional optimization local interval [lower, upper[
 *
 * \param e The environment in which to operate.
 * \param len The size of terms.
 ...
 * in case of errors.
 */
msat_objective msat_push_maxmin(msat_env e, size_t len, msat_term terms[],
                                msat_term lower, msat_term upper,
                                bool bvsigned = false);
...
/*@}*/ /* end of objective creation group */
 ...
 *
 * The \a func function will be invoked each time a new model is found by the
 * pareto optimization search. If the function returns zero, then the current
 ...
 * \return zero on success, nonzero on error
 */
int msat_set_pareto_model_callback(msat_env env,
 ...
 * instanciated after a ::msat_solve call, and prior to any further
 ...
 * \param o The objective providing the value.
 * \param i The objective field to be tested.
 * \param d Output delta component: -1 if negative, 1 if positive and 0 if not strict
 * \return 1 if strict, 0 if not, -1 on error.
 */
int msat_objective_value_is_strict(msat_env e, msat_objective o,
                                   msat_objective_value i, int &d);
 ...
 * \param fin If true a finite value representation is returned, instead of
 *          an MSAT_ERROR_TERM, for any unbounded field
 * \param eps if true any strict value will be converted into a non-strict value
 *          by adding a configurable epsilon amount. Otherwise, the delta component
 *          of any strict value is ignored and its non-strict approximation
...
msat_term msat_objective_value_term(msat_env e, msat_objective o, msat_objective_value i,
                                    bool fin = false, bool eps = false);

/**
 * \brief Returns a string representation of the given objective value.
 * \param e The environment in which to operate.
 * \param o The objective providing the value.
 * \param i The objective field to retrieve.
